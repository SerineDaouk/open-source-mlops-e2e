...
def load_train_params(params):
    ...
    return processed_data_dir, random_state, feat_cols, model_type, train_params
    
def create_model_dir(params):
    model_dir = Path(params.train.model_dir)
    model_dir.mkdir(exist_ok=True)
    model_path = Path(params.train.model_path)
    return model_path

def load_training_data(processed_data_dir: str):
...
def train(params):
    ...
    processed_data_dir, random_state, feat_cols, model_type, train_params = load_train_params(params)
    
    # Create model path
    model_path = create_model_dir(params)
